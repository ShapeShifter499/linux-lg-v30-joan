...
struct pmic_typec_resources {
	...
	/* For ports that do not follow the PM8150B register layout */
...
static int qcom_pmic_typec_probe(struct platform_device *pdev)
{
	...
	if (res->port_probe)
		ret = res->port_probe(pdev, tcpm, regmap, base);
	else
...
static const struct pmic_typec_resources pmi8998_typec_res = {
	/* The PD PHY is the PM8150B one, with the same seven interrupts */
	.pdphy_res = &pm8150b_pdphy_res,
	.port_probe = qcom_pmic_typec_pmi8998_port_probe,
};
