 ...
 * Split out of qcom-cpufreq-hw for MSM8998, whose OSM must be fully
 * programmed by the kernel, and ported to the current OPP/genpd APIs.
...
static const char * const cprh_pd_names[] = { "perf" };

static const struct dev_pm_domain_attach_data cprh_pd_data = {
	.pd_names = cprh_pd_names,
	.num_pd_names = ARRAY_SIZE(cprh_pd_names),
				   ...
				   u8 *num_entries)
{
	...
	ret = devm_pm_domain_attach_list(cpu_dev, &cprh_pd_data, &pd_list);
	if (ret < 0) {
		dev_err(&pdev->dev, "Could not attach to pm_domain: %d\n", ret);
		return ret;
	}
	if (!pd_list || pd_list->num_pds < 1 || !pd_list->pd_devs[0])
		return -EINVAL;
	 ...
	 */
	cpr_data = dev_get_drvdata(pd_list->pd_devs[0]);
	if (cpr_data == NULL) {
...
MODULE_DESCRIPTION("Qualcomm MSM8998 OSM CPUFreq driver (CPRh aware)");
